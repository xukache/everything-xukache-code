...
B 端项目必须默认读取 `assets/design-themes/vben/DESIGN.md`，但只用于提取 Vben 主色和品牌色 token。组件框架、页面骨架、布局模式、表格、筛选、表单、弹窗、状态、导航和交互必须完全按照 Arco Design Pro Vue / Arco Design Vue 一比一引用和复用。若项目技术栈不是 Vue，也要按 Arco Design Pro 的信息架构、布局、密度、组件语义和交互模式在当前技术栈中复刻；只有用户明确拒绝或技术架构存在硬冲突时，才能不使用 Vben 主色。

## 使用原则

- B 端网页默认应用本参考，特殊业务场景可在 `docs/ui-design.md` 中说明偏离原因。
- B 端默认使用 Vben 主色 + Arco Design Pro 组件；偏离时必须在 `docs/ui-design.md` 和 `docs/ui-design-tokens.md` 记录原因。
