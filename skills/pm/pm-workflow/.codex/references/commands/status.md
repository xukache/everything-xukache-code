...
汇报"推荐下一步"时按 2.0 阶段顺序判断:`init → blueprint → design → analyze(PRD,后置) → architect → plan → deliver`。蓝图(`docs/feature-flow-layout.md`)是 init 与 design 之间的桥梁;PRD 后置成文,基于已确认的蓝图与 UI 定稿回填。

## 必须执行的流程

1. 总结当前阶段、已有产物、审核轮次、最近审核结论和推荐下一步(对照 2.0 阶段顺序)。
