...
开始自审前必须确认 `.kiro/skills/impeccable/SKILL.md` 存在；不存在时停止，不做普通降级。

1. 生成 Impeccable 上下文：写入 `.kiro/context/PRODUCT.md` 和 `.kiro/context/DESIGN.md`。
2. 运行 `node .kiro/skills/impeccable/scripts/load-context.mjs`，完整读取输出。
