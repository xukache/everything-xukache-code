...
3. 从 `assets/design-themes/` 中选定主题；中后台、控制台、CRM、运营平台、内部工具、权限/表格/表单密集系统默认优先 Vben；用户指定品牌、参考站点、截图或风格名时，可查找 `assets/design-themes/open-design/<slug>/DESIGN.md`。
4. 读取被选中的主题文件，并把 token、组件、布局和 Do/Don't 作为 HTML 原型的主约束。
5. 把产品语境、主题、源文件、选择原因和补充建议写入 `notes/requirements.md` 的“原型设计输入”。
6. 再开始 HTML 原型设计。
