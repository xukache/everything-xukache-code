...
- `docs/feature-flow-layout.md` (blueprint — Mx-Fx feature ID source)
- `docs/prd.md` (post-design PRD)
...
If task planning reveals missing or changed environment decisions, scaffold method, framework versions, module boundaries, interfaces, test strategy, requirements, architecture constraints, page paths, acceptance criteria, or technical constraints, run the major-change protocol: validate insight → map cascade → confirm scope → propagate consistently (write back to blueprint / PRD / architecture / UI as needed) → consistency check. Do not silently introduce new requirements only in `docs/dev-tasks.md`. As of 2.0 we no longer require a fixed `## 文档同步检查` table; `review plan` performs the consistency backstop.
...
Every task must end with a requirement trace line such as `_需求: M1-F1, M2-F3_` (reuse the blueprint layer-4 feature IDs verbatim — do not renumber); engineering setup tasks may use `_需求: 工程准备_`.
