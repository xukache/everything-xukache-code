...
4. 文档同步硬门禁：检查当前阶段产物是否包含有效 `## 文档同步检查`，并验证上下游文档已同步或有明确不需要同步原因。
...
- 每个阶段是否包含有效 `## 文档同步检查` 表，列包含变更项、影响类型、是否影响上游事实、已检查文档、已同步文档、不需要同步原因、责任阶段、检查结论。
- 下游文档如改变上游事实，是否同步回写上游源文档、当前阶段文档和下游交接文档，并在 `workflow-state.json` notes 记录同步说明。
- 同步检查是否存在全空、`待补充`、泛泛“不适用”或高风险变更未说明同步结果的情况；命中则审核不通过。
- 是否存在跨阶段新增范围。
