...
3. 采用两步式流程：先生成符合新 1-8 章格式的 PRD 草稿，未解决问题只写入 `docs/workflow-state.json.pending_user_questions`，不写进 PRD 正文。
4. 草稿阶段不触发审核；最终稿完成后清空 `pending_user_questions`，把 `recommended_next` 设置为 `review analyze`，并自动触发 `quality_reviewer` 审核 analyze。
5. 做四轮引导并映射到新 PRD：产品概述与目标用户、功能范围、核心业务流程、功能详细设计/规则/异常/接口、数据模型/权限/非功能。
...
10. P0 功能必须有业务规则、页面字段、页面操作、状态流转、权限、异常边界、初步接口需求和验收信号。
11. 明确不在范围内的功能，防止范围失控。
