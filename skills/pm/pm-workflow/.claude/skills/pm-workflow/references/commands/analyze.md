...
必须启动当前 CLI 结构下的需求分析子 agent 执行 PRD 撰写:Codex 使用 `.codex/agents/demand-analyst.toml`,Claude Code 使用 `.claude/agents/demand-analyst.md`。

如果当前环境无法启动需求分析子 agent,必须停止本阶段,不生成或修改 `docs/prd.md`、`docs/handoff-prd.md` 和 `docs/workflow-state.json`,并提示用户在支持项目子 agent 调度的 CLI 中打开当前工作室目录后重试。
...
10. **最终稿完成后自动启动 `quality_reviewer` 执行 `$pm-workflow review analyze`**。
