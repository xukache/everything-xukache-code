...
必须启动 `.codex/agents/product-manager.toml` 中的 `product_manager` 子 agent 汇总当前状态。

如果当前 Codex 环境无法启动 `product_manager` 子 agent，必须停止状态汇总，不修改任何产物，并提示用户在支持项目子 agent 调度的 Codex 运行方式中打开当前工作室目录后重试。
